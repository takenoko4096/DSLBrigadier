...
@BrigadierDSL
class SuggestibleCommandNode<S, T>(override val argumentBuilder: RequiredArgumentBuilder<S, T>) : DSLCommandNode<S>(argumentBuilder) {
        ...
        fun strings(vararg values: String, tooltipBuilder: ((String) -> Component)? = null) {
            if (tooltipBuilder == null) {
                ...
                values.forEach {
                    this.string(
                        it,
                        MessageComponentSerializer.message().serialize(tooltipBuilder(it))
                    )
                }
            }
        }
